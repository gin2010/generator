...
class Generator(object):
    ...
    def generate_request_json(self,temp,data):
        ...
        if isinstance(data[2], str):
            ...
            len_and_digit = data[2].replace("，", ",").split(",")
        else:
            len_and_digit = []
            # data.pop()
            # data.extend(len_and_digit)
        # if len(data) == 4:
        ...
        if len(len_and_digit) == 2:
            string_list = get_string.random_string_main(data[1],int(len_and_digit[0]),int(len_and_digit[1]))
        else:
            string_list = get_string.random_string_main(data[1], int(data[2]))
        # 添加data中传入的固定值，描述统一用“输入值xxx”
        if data[3]:
            fix_values = data[3].replace("，", ",").split(",")
            print(data)
            print(fix_values)
            for x in fix_values:
